...
class CleanRPCResponseCallback(val corrId: String) extends DeliverCallback {
...
class RPCClientCleanTimeout(host: String) {
  ...
  def call(message: String, timeoutMillis: Long = 5000): String = {
    ...
    val props = new BasicProperties.Builder().correlationId(corrId)
      ...
      .expiration(timeoutMillis.toString)
      .build()
    channel.basicPublish("", requestQueueName, props, message.getBytes("UTF-8"))

    val responseCallback = new CleanRPCResponseCallback(corrId)
    ...
    maybeResponse.getOrElse{
      // alt solution: tell broker to discard the req message from queue
...
object RPCClientCleanTimeout {

  def main(argv: Array[String]) {
    var fibonacciRpc: RPCClientCleanTimeout = null
    var response: String = null
    try {
      val host = if (argv.isEmpty) "localhost" else argv(0)

      fibonacciRpc = new RPCClientCleanTimeout(host)
      println(" [x] Requesting fib(30) and waitin for 3 seconds")
      response = fibonacciRpc.call("30", 3000)
      println(" [.] Got '" + response + "'")
    } catch {
      case e: java.util.concurrent.TimeoutException => println(" [!] Request timed out (3 seconds) and discarded the request from the queue")
